...
uint8_t BNO055::getCalibrationRaw() {
  uint8_t calData = read8(BNO055_CALIB_STAT_ADDR);
  return calData;
}
...
*/
/**************************************************************************/
IntVector<3> BNO055::getVector(bno_vector_type_t vector_type)
{
  uint8_t buffer[6];
  memset (buffer, 0, 6);
...
//   IntVector<3> euler = getVector(BNO055::VECTOR_EULER);
