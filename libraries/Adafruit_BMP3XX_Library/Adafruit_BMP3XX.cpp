...
#else
int64_t Adafruit_BMP3XX::readTemperature(void) {
  performReading();
  return temperature;
}
#endif
...
#else
uint64_t Adafruit_BMP3XX::readPressure(void) {
  performReading();
  return pressure;
}
#endif
...
float Adafruit_BMP3XX::calculateAltitude(float seaLevel) {
  // Equation taken from BMP180 datasheet (page 16):
  //  http://www.adafruit.com/datasheets/BST-BMP180-DS000-09.pdf
...
//   rslt = bmp3_get_sensor_data_raw(sensor_comp, &data, &the_sensor, &uncomp_data);
  if (rslt != BMP3_OK)
    return false;
...
//   uncomp_temperature = uncomp_data.temperature;
//   uncomp_pressure = uncomp_data.pressure;
